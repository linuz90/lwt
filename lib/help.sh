...
lwt::ui::help_add() {
  ...
  echo "  ${_lwt_bold}--tab-dev${_lwt_reset}               ${_lwt_dim}Run the repo's dev command in a new terminal tab after setup${_lwt_reset}"
  echo "  ${_lwt_bold}--tab-claude [prompt]${_lwt_reset}   ${_lwt_dim}Launch Claude in a tab${_lwt_reset}"
  echo "  ${_lwt_bold}--tab-codex [prompt]${_lwt_reset}    ${_lwt_dim}Launch Codex in a tab${_lwt_reset}"
  echo "  ${_lwt_bold}--tab-gemini [prompt]${_lwt_reset}   ${_lwt_dim}Launch Gemini in a tab${_lwt_reset}"
  ...
  echo "  ${_lwt_dim}Tab helpers mirror split helpers for common dev and single-agent launches.${_lwt_reset}"
  echo "  ${_lwt_dim}--split-dev and --tab-dev also force setup before launching the dev command.${_lwt_reset}"
