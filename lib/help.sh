lwt::ui::help_main() {
  ...
  echo "  lwt a feat-auth-alt --from feat-auth         ${_lwt_dim}Branch from an explicit ref instead of the default branch${_lwt_reset}"
  ...
  echo "  lwt a feature-alt --from feature           ${_lwt_dim}Create a new branch from feature${_lwt_reset}"
  echo "  lwt a feature-alt --from-current           ${_lwt_dim}Create a new branch from the current worktree${_lwt_reset}"
...
lwt::ui::help_automation() {
  ...
  echo "  lwt a feat-auth                            ${_lwt_dim}Create from the repo default branch, or re-use feat-auth if it already exists${_lwt_reset}"
  echo "  lwt a feat-auth-alt --from feat-auth       ${_lwt_dim}Create a new branch from an explicit ref${_lwt_reset}"
...
lwt::ui::help_add() {
  ...
  echo "  ${_lwt_bold}--from <ref>${_lwt_reset}             ${_lwt_dim}Create a new branch from an explicit ref instead of the default base${_lwt_reset}"
  echo "  ${_lwt_bold}--from-current${_lwt_reset}           ${_lwt_dim}Create a new branch from the current worktree's HEAD${_lwt_reset}"
  ...
  echo
  lwt::ui::header "Common Flows"
  echo "  lwt a new-branch                           ${_lwt_dim}New branch from the repo default branch${_lwt_reset}"
  echo "  lwt a existing-branch                      ${_lwt_dim}Existing local/origin branch into a worktree${_lwt_reset}"
  echo "  lwt a feat-auth-alt --from feat-auth       ${_lwt_dim}New branch from feat-auth${_lwt_reset}"
  echo "  lwt a feat-auth-alt --from-current         ${_lwt_dim}New branch from the current worktree's HEAD${_lwt_reset}"
  echo
  lwt::ui::header "Notes"
